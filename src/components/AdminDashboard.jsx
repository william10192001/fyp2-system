...
const S = {
  ...
  badge: (c) => ({ background: `${c}20`, color: c, border: `1px solid ${c}40`, fontSize: 11, fontWeight: 600, padding: "3px 10px", borderRadius: 99, display: "inline-block" }),
  pill:  (active, c) => ({
  ...
  return (
    <div style={S.page}>
      ...
      <div style={{ padding: "32px", maxWidth: 1100, margin: "0 auto" }}>
            ...
            <div style={S.card}>
              ...
              {[...candidates, ...employers].sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt)).slice(0, 8).map((u, i) => (
                <div key={i} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "10px 0", borderBottom: "1px solid #1f2937" }}>
                  <div><div style={{ fontSize: 13, fontWeight: 600, color: "#e2e8f0" }}>{u.name || u.email}</div><div style={{ fontSize: 11, color: "#6b7280" }}>{u.email}</div></div>
        ...
        {/* ════ CANDIDATES (with inline applications) ════ */}
        {tab === "candidates" && (
          <>
            ...
            {loading ? <div style={{ color: "#475569", textAlign: "center", padding: 40 }}>Loading...</div> :
              candidates
                ...
                .map((c, i) => {
                  const candApps = applications.filter(a => a.candidateEmail === c.email);
                        ...
                        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                          <div style={{ flex: 1 }}>
                            <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 10 }}>
                              <div style={{ width: 38, height: 38, borderRadius: "50%", background: "linear-gradient(135deg,#2563eb,#7c3aed)", display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 700, fontSize: 14, flexShrink: 0 }}>
                                {(c.name||c.email)[0].toUpperCase()}
                              </div>
                              <div>
                                <div style={{ fontWeight: 700, fontSize: 15, color: "#e2e8f0" }}>{c.name || "—"}</div>
                                <div style={{ fontSize: 12, color: "#6b7280" }}>{c.email}</div>
                              </div>
                              {candApps.length > 0 && (
                                <span style={{ background: "rgba(37,99,235,0.15)", color: "#60a5fa", border: "1px solid rgba(37,99,235,0.3)", fontSize: 11, fontWeight: 600, padding: "2px 8px", borderRadius: 99 }}>
                                  {candApps.length} application{candApps.length > 1 ? "s" : ""}
                                </span>
                              )}
                            </div>
                          ...
                          <div style={{ display: "flex", gap: 8, marginLeft: 16, flexShrink: 0 }}>
                            <button onClick={() => setExpanded(isExpanded ? null : c.email)} style={{ background: isExpanded ? "rgba(37,99,235,0.15)" : "#1e293b", color: isExpanded ? "#60a5fa" : "#94a3b8", border: `1px solid ${isExpanded ? "rgba(37,99,235,0.3)" : "#334155"}`, borderRadius: 8, padding: "7px 14px", fontSize: 12, cursor: "pointer" }}>
                              {isExpanded ? "▲ Hide" : "▼ Details"}
                            </button>
                            <button onClick={() => delUser(c.email)} style={{ background: "rgba(239,68,68,0.1)", color: "#f87171", border: "1px solid rgba(239,68,68,0.2)", borderRadius: 8, padding: "7px 14px", fontSize: 12, cursor: "pointer" }}>🗑️</button>
                          </div>
                        </div>
                      </div>

                      {/* Expanded: keywords + applications */}
                      {isExpanded && (
                        <div style={{ borderTop: "1px solid #1f2937", background: "#0f172a" }}>
                          ...
                          <div style={{ padding: "16px 24px", borderBottom: "1px solid #1f2937" }}>
                            <div style={{ fontSize: 11, color: "#475569", fontWeight: 600, textTransform: "uppercase", letterSpacing: 1, marginBottom: 10 }}>
                              📄 Resume Keywords ({(c.resumeKeywords||[]).length})
                            </div>
                            <div style={{ display: "flex", flexWrap: "wrap", gap: 5, maxHeight: 80, overflowY: "auto" }}>
                              {(c.resumeKeywords||[]).map((kw, j) => (
                                <span key={j} style={{ background: "rgba(37,99,235,0.12)", color: "#60a5fa", fontSize: 10, padding: "2px 8px", borderRadius: 99 }}>{kw}</span>
                              ))}
                              {(c.resumeKeywords||[]).length === 0 && <span style={{ color: "#374151", fontSize: 12 }}>No keywords — resume not uploaded</span>}
                            </div>
                          </div>
                          ...
                          <div style={{ padding: "16px 24px" }}>
                            <div style={{ fontSize: 11, color: "#475569", fontWeight: 600, textTransform: "uppercase", letterSpacing: 1, marginBottom: 10 }}>
                              📥 Applications ({candApps.length})
                            </div>
                            {candApps.length === 0 ? (
                              <p style={{ color: "#374151", fontSize: 12, margin: 0 }}>No applications yet.</p>
                            ) : candApps.map((app, k) => {
                              ...
                              return (
                                <div key={k} style={{ background: "#111827", border: "1px solid #1f2937", borderRadius: 10, padding: "12px 16px", marginBottom: 8, display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 12 }}>
                                  <div style={{ flex: 1 }}>
                                    <div style={{ fontWeight: 600, color: "#e2e8f0", fontSize: 13, marginBottom: 4 }}>{app.jobTitle || "Job"}</div>
                                    <div style={{ fontSize: 11, color: "#6b7280", display: "flex", gap: 12, flexWrap: "wrap" }}>
                                      ...
                                      {app.matchScore > 0 && <span style={{ color: scoreColor(app.matchScore), fontWeight: 600 }}>{app.matchScore}% match</span>}
                                    </div>
                                  </div>
                                  <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                                    <span style={S.badge(cfg.color)}>{cfg.label}</span>
                                    <button onClick={() => delApp(app._id)} style={{ background: "rgba(239,68,68,0.1)", color: "#f87171", border: "1px solid rgba(239,68,68,0.2)", borderRadius: 6, padding: "3px 8px", fontSize: 11, cursor: "pointer" }}>🗑️</button>
                                  </div>
                                </div>
                              );
                            })}
            ...
            {/* ── Global Applications filter ── */}
            <div style={{ marginTop: 32, marginBottom: 16 }}>
              <h3 style={{ fontSize: 16, fontWeight: 700, marginBottom: 16, color: "#94a3b8" }}>📥 All Applications ({applications.length})</h3>
              ...
              {filteredApps.length === 0 ? (
                <div style={{ textAlign: "center", padding: "30px 20px", color: "#374151" }}>
                  ...
                  {appFilter !== "all" && <button onClick={() => setAppFilter("all")} style={{ color: "#60a5fa", background: "none", border: "none", cursor: "pointer", fontSize: 13 }}>Clear filter</button>}
                </div>
        ...
        {/* ════ EMPLOYERS & JOBS (combined) ════ */}
        {tab === "employers" && (
          <>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
              <h2 style={{ fontSize: 22, fontWeight: 700, margin: 0 }}>Employers & Jobs</h2>
              <input placeholder="🔍 Search by email or job title..." value={search} onChange={e => setSearch(e.target.value)} style={{ ...S.input, width: 280 }} />
            </div>
            {loading ? <div style={{ color: "#475569", textAlign: "center", padding: 40 }}>Loading...</div> :
              employers
                .filter(emp => !search || emp.email.toLowerCase().includes(search.toLowerCase()) || jobs.some(j => j.employerEmail === emp.email && j.jobTitle?.toLowerCase().includes(search.toLowerCase())))
                .map((emp, i) => {
                  const empJobs = jobs.filter(j => j.employerEmail === emp.email);
                  return (
                    <div key={i} style={{ ...S.card, padding: 0, overflow: "hidden", marginBottom: 16 }}>
                      {/* Employer header */}
                      <div style={{ padding: "20px 24px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                          <div style={{ width: 44, height: 44, borderRadius: 12, background: "linear-gradient(135deg,#7c3aed,#2563eb)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 20, flexShrink: 0 }}>🏢</div>
                          <div>
                            <div style={{ fontWeight: 700, fontSize: 15, color: "#e2e8f0" }}>{emp.email}</div>
                      ...
                      {/* Each job with its keywords */}
                      {empJobs.length > 0 && (
                        <div style={{ borderTop: "1px solid #1f2937" }}>
                          {empJobs.map((job, k) => (
                            <div key={k} style={{ padding: "16px 24px", borderBottom: k < empJobs.length - 1 ? "1px solid #1f2937" : "none", background: "#0f172a" }}>
                              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 12 }}>
                                <div style={{ flex: 1 }}>
                                  <div style={{ fontWeight: 700, fontSize: 14, color: "#e2e8f0", marginBottom: 4 }}>{job.jobTitle}</div>
                                  ...
                                  {(job.jobKeywords||[]).length > 0 && (
                                    <div>
                                      <div style={{ fontSize: 10, color: "#475569", fontWeight: 600, textTransform: "uppercase", letterSpacing: 1, marginBottom: 6 }}>
                                        🔑 Keywords ({job.jobKeywords.length})
                                      </div>
                                      <div style={{ display: "flex", flexWrap: "wrap", gap: 5 }}>
                                        {job.jobKeywords.slice(0, 12).map((kw, m) => (
                                          <span key={m} style={{ background: "rgba(124,58,237,0.12)", color: "#a78bfa", fontSize: 10, padding: "2px 8px", borderRadius: 99 }}>{kw}</span>
                                        ))}
                                        {job.jobKeywords.length > 12 && <span style={{ color: "#374151", fontSize: 10 }}>+{job.jobKeywords.length - 12} more</span>}
                                      </div>
                                    </div>
                                  )}
                                </div>
                                <button onClick={() => delJob(job._id)} style={{ background: "rgba(239,68,68,0.1)", color: "#f87171", border: "1px solid rgba(239,68,68,0.2)", borderRadius: 8, padding: "6px 12px", fontSize: 11, cursor: "pointer", flexShrink: 0 }}>🗑️</button>
                              </div>
                            </div>
                          ))}
                        </div>
