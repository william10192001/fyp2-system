...
function CandidateDashboard({ user, logout }) {
  ...
  const selectJob = async (job) => {
    setSelectedJob(job); setAiAnalysis(null); setActiveDetail("description"); setAnalyzing(true);
    try {
      const res  = await fetch(`${BASE}/analyze-job-match`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ candidateEmail: user.email, jobId: job.jobId }) });
      const data = await res.json();
      setAiAnalysis(data);
    } catch (e) { console.log(e); }
    setAnalyzing(false);
  ...
  const applyToJob = async (job) => {
    ...
    try {
      const res  = await fetch(`${BASE}/apply`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ jobId: job.jobId, candidateEmail: user.email }) });
      const data = await res.json();
      if (res.ok) {
      ...
      } else { setApplyingId(null); alert(data.msg); }
    } catch { setApplyingId(null); alert("Apply failed ❌"); }
  };

  const toggleSaveJob = async (jobId) => {
    const isSaved = savedJobs.includes(jobId);
    try {
      await fetch(`${BASE}/toggle-save-job`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ jobId, candidateEmail: user.email, action: isSaved ? "unsave" : "save" }) });
    ...
    <div style={S.page}>
      ...
      {hasResume && (
        <div style={{ background: "linear-gradient(90deg,#111827,#1e293b)", padding: "13px 32px", display: "flex", justifyContent: "space-between", alignItems: "center", borderBottom: "1px solid #1f2937" }}>
          <p style={{ color: "#e2e8f0", fontSize: 13, fontWeight: 500, margin: 0 }}>Hello {candidateName}! Showing personalized job recommendations sorted by your resume match.</p>
          <div style={{ background: "rgba(37,99,235,0.2)", border: "1px solid rgba(37,99,235,0.3)", padding: "5px 14px", borderRadius: 99, color: "#60a5fa", fontSize: 12 }}>📄 Resume uploaded ✓</div>
        </div>
      )}
        ...
        <>
          <div style={{ background: "#111827", borderBottom: "1px solid #1f2937", padding: "10px 32px", display: "flex", alignItems: "center", gap: 12, position: "relative" }}>
            <div style={{ position: "relative" }}>
              <button onClick={() => setShowFilter(p => !p)} style={{ fontSize: 13, padding: "6px 14px", borderRadius: 8, cursor: "pointer", border: showFilter ? "1px solid #2563eb" : "1px solid #334155", background: showFilter ? "rgba(37,99,235,0.15)" : "#1e293b", color: showFilter ? "#60a5fa" : "#94a3b8", display: "flex", alignItems: "center", gap: 6 }}>
                ⚡ Filters {filterLevel !== "All" && <span style={{ background: "#2563eb", color: "white", borderRadius: 99, padding: "0px 7px", fontSize: 11 }}>1</span>}
              </button>
              {showFilter && (
                <div style={{ position: "absolute", top: "calc(100% + 6px)", left: 0, zIndex: 200, background: "#1e293b", border: "1px solid #334155", borderRadius: 12, boxShadow: "0 12px 32px rgba(0,0,0,0.4)", padding: 8, minWidth: 200 }}>
                  <div style={{ padding: "4px 12px 8px", fontSize: 11, color: "#475569", fontWeight: 600, textTransform: "uppercase" }}>Match Level</div>
                  {["All","Perfect Match","Good Match","Partial Match","Weak Match"].map(level => (
                    <div key={level} onClick={() => { setFilterLevel(level); setShowFilter(false); }} style={{ padding: "9px 12px", cursor: "pointer", borderRadius: 8, fontSize: 13, background: filterLevel === level ? "rgba(37,99,235,0.15)" : "transparent", color: filterLevel === level ? "#60a5fa" : "#94a3b8", fontWeight: filterLevel === level ? 600 : 400 }}>
                      {level === "All" ? "All Matches" : level}{filterLevel === level && <span style={{ float: "right" }}>✓</span>}
                    </div>
                  ))}
                </div>
              )}
          ...
          <div key={i} style={{ background: "#111827", border: "1px solid #1f2937", borderRadius: 14, padding: "20px 24px", marginBottom: 14 }}>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 14 }}>
              <div>
                ...
                <div style={{ fontSize: 11, color: "#374151", marginTop: 4 }}>📅 {new Date(a.createdAt).toLocaleDateString("en-MY", { year: "numeric", month: "long", day: "numeric" })}</div>
                {a.status === "rejected" && <div style={{ fontSize: 11, color: "#f87171", marginTop: 4 }}>💡 You can re-apply from Job Matches</div>}
              </div>
              <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 8 }}>
                <span style={{ background: s.bg, color: s.color, border: `1px solid ${s.border}`, fontSize: 12, fontWeight: 600, padding: "5px 14px", borderRadius: 99 }}>{s.label}</span>
                {a.status === "pending" && (
                  <button onClick={() => cancelApp(a._id)} style={{ background: "none", border: "1px solid rgba(239,68,68,0.3)", color: "#f87171", fontSize: 11, padding: "4px 12px", borderRadius: 8, cursor: "pointer" }}>Cancel</button>
                )}
              </div>
            </div>
